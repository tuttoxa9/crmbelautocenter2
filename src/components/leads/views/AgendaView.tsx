    ...
    return (
      <div
        ...
        className={`p-4 rounded-[1.25rem] border transition-all duration-300 flex flex-col gap-2.5 relative cursor-pointer shadow-sm ${
          isSelected
            ? "bg-[#141416] border-blue-400 shadow-[0_8px_24px_-8px_rgba(59,130,246,0.2)] scale-[0.98]"
            : "bg-[#141416] hover:bg-[#141416] border-white/10 hover:shadow-md hover:border-white/20"
        }`}
      >
        <div className="flex justify-between items-start gap-2">
          <div className="flex flex-col min-w-0">
            <span className={`font-semibold text-[15px] truncate tracking-tight ${isSelected ? "text-blue-300" : "text-zinc-100"}`}>
        ...
        {(lead.car || lead.notes) && (
          <div className="bg-white/[0.04] rounded-xl p-2.5 text-[12px] flex flex-col gap-1 mt-1">
            {lead.car && (
    ...
    return (
      <div
        ...
        className={`flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-3 sm:px-5 sm:py-3.5 rounded-[1.25rem] border transition-all duration-300 relative cursor-pointer group ${
          isSelected
            ? "bg-[#141416] border-blue-300 shadow-[0_4px_20px_-4px_rgba(59,130,246,0.12)] z-10"
            : "bg-[#141416]/90 hover:bg-[#141416] border-transparent hover:border-white/10 hover:shadow-sm"
        }`}
      >
        ...
        <div className="w-full sm:w-[35%] shrink-0 flex flex-col min-w-0 gap-0.5 justify-center">
           {lead.car && <span className="text-[13px] font-medium text-zinc-200 truncate">{lead.car}</span>}
  ...
  return (
    <div className="flex h-full w-full bg-black overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col min-w-0 z-10 relative">
        ...
        {/* Top Header & Actions */}
        <div className="h-16 px-4 md:px-6 bg-[#141416]/95 backdrop-blur-xl border-b border-black/5 flex items-center justify-between sticky top-0 shrink-0 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
          <div className="flex items-center gap-4">
            <QuickAddLead cars={[]} allLeads={[]} />
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-white/[0.08] rounded-lg">
              <CalendarIcon className="w-4 h-4 text-zinc-400" />
              <h2 className="text-sm font-bold text-zinc-300 tracking-tight">Повестка дня</h2>
            </div>
          </div>
          
          <div className="flex items-center gap-1 bg-[#141416] border border-white/10 shadow-sm rounded-[10px] p-1">
            <button
              onClick={handlePrevDay}
              className="p-1.5 hover:bg-white/[0.08] text-zinc-400 rounded-lg transition-colors"
            >
            ...
            <button
              ...
              className={`px-4 py-1.5 text-[13px] font-bold min-w-[120px] text-center rounded-lg transition-all ${
                isToday(filterDate) ? "text-blue-300 bg-blue-500/15 shadow-sm" : "text-zinc-300 hover:bg-white/[0.06]"
              }`}
            >
          ...
          <div className="max-w-4xl mx-auto space-y-10 pb-20">
            ...
            {agendaGroups.length === 0 && (
              <div className="flex flex-col items-center justify-center py-20 px-4">
                <div className="w-16 h-16 bg-[#141416] rounded-3xl shadow-sm border border-white/10 flex items-center justify-center mb-4">
                  <CalendarIcon className="w-7 h-7 text-zinc-300" />
                </div>
                <h3 className="text-[17px] font-bold text-zinc-200">Нет задач на этот день</h3>
            ...
            {agendaGroups.map((group) => (
              <div key={group.status} className="space-y-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center gap-2.5 ml-2">
                  ...
                  <span className="bg-white/[0.08] text-zinc-500 px-2 py-0.5 rounded-full text-[10px] font-bold">{group.items.length}</span>
                </div>
                <div className="flex flex-col gap-1.5 bg-[#141416] p-1.5 rounded-[1.5rem] border border-white">
      ...
      <div className="hidden lg:flex w-[340px] shrink-0 my-5 mr-5 rounded-[2rem] shadow-[0_12px_40px_-12px_rgba(0,0,0,0.12)] bg-[#141416] backdrop-blur-2xl border border-white flex-col z-20 overflow-hidden relative">
        ...
        <div className="h-16 px-6 relative flex items-center justify-between shrink-0 border-b border-black/[0.03]">
          <h2 className="text-[13px] font-bold uppercase tracking-widest text-zinc-100 flex items-center gap-2.5">
            <div className="relative flex h-2.5 w-2.5">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-blue-500/150"></span>
            </div>
            Новые
          </h2>
          <span className="bg-blue-100 text-blue-300 px-2.5 py-0.5 rounded-full text-[11px] font-black shadow-sm">
            {newLeads.length}
          </span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-3 relative">
          {newLeads.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-40 text-center px-4 opacity-60">
               <div className="w-12 h-12 rounded-2xl bg-white/[0.08] border border-white/10 flex items-center justify-center mb-3 shadow-inner">
                  <Inbox className="w-5 h-5 text-zinc-400" />
               </div>
              <p className="text-sm font-medium text-zinc-400">Отличная работа!</p>
