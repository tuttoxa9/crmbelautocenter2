...
export function LeadFocusView({ lead, cars, allLeads, onClose, onOpenCar, onDeleted, onOpenDuplicate }: LeadFocusViewProps) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex justify-end">
      ...
      <div className="relative flex h-full w-full max-w-[720px] flex-col bg-[#141416] shadow-2xl md:rounded-l-[1.75rem]">
        {picker ? (
          <CarPicker
            cars={cars}
        ...
        <div className="flex items-center justify-between border-b border-leads-line px-3 py-2.5">
          <button type="button" onClick={onClose} className="flex items-center gap-1 rounded-full px-2 py-1.5 text-[13px] font-medium text-leads-muted hover:bg-white/[0.08] hover:text-leads-ink">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => setConfirmDelete(true)} className="flex size-9 items-center justify-center rounded-full text-leads-subtle hover:bg-red-500/15 hover:text-red-600">
        ...
        <AdsScroller className="min-h-0 flex-1" contentClassName="px-5 py-5 md:px-8 md:py-7">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            {tel.length >= 9 ? (
              <a href={`tel:+${tel}`} className="rounded-full bg-white px-3 py-1.5 text-[12px] font-semibold text-black">
                Позвонить
              </a>
            ) : null}
          </div>
          ...
          {duplicate ? (
            <button
              ...
              className="mt-3 w-full rounded-xl bg-amber-500/15 px-3 py-2 text-left text-[12px] text-amber-200 ring-1 ring-amber-400/40 transition-colors hover:bg-amber-500/20"
            >
              Этот номер уже есть: {duplicate.name || "без имени"} · {getStatusLabel(duplicate.status)}
              <span className="mt-0.5 block text-[11px] text-amber-200/80">Открыть в {getStatusLabel(duplicate.status)}</span>
            </button>
          ) : null}
          ...
          <div className="mt-8">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="h-11 w-full rounded-xl bg-white/[0.04] px-3 text-[13px] outline-none ring-1 ring-leads-line focus:bg-white/[0.06]"
              />
            </div>
          </div>
          ...
          <div className="mt-8">
            ...
            <textarea
              ...
              rows={5}
              className="w-full resize-y rounded-2xl bg-white/[0.04] p-3 text-[13px] leading-relaxed outline-none ring-1 ring-leads-line focus:bg-white/[0.06]"
        ...
        {dirty ? (
          <div className="border-t border-leads-line bg-[#141416]/95 p-4 backdrop-blur">
        ...
        {confirmDelete ? (
          <div className="absolute inset-0 z-[90] flex items-end justify-center bg-black/70 p-4 md:items-center">
            <div className="w-full max-w-sm rounded-3xl bg-[#141416] p-5">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        highlight ? "bg-amber-500/15 ring-2 ring-inset ring-amber-400" : selected ? "bg-white/[0.08]" : "hover:bg-white/[0.04]",
      )}
    >
      ...
      <div className="hidden min-w-0 max-w-[220px] shrink-0 sm:block">
        {car ? (
          ...
          >
            <span className="size-8 overflow-hidden rounded-lg bg-white/[0.08]">
              {car.photoUrl ? (
      ...
      {when ? (
        <span className="shrink-0 rounded-full bg-white/[0.08] px-2 py-1 text-[11px] font-medium text-leads-ink">
          {when}
        </span>
      ) : null}
    </button>
