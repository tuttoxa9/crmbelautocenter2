...
import { leadActionAt } from "@/lib/leads/time";
...
export function buildDayGroups(leads: Lead[], tab: DayTab, filterDate: Date, search: string): BoardGroup[] {
  ...
  rows = rows.filter((lead) => {
    const d = startOfDay(new Date(leadActionAt(lead)));
  ...
  for (const lead of rows) {
    ...
    const d = startOfDay(new Date(at));
  ...
  const overdueDays = Object.values(overdueByDay)
    .map((g) => ({
      ...g,
      items: g.items.sort((a, b) => leadActionAt(b) - leadActionAt(a)),
    }))
...
export function DayBoard({
  ...
  const groups = useMemo(
    () => buildDayGroups(leads, tab, filterDate, search),
                ...
                <div className="divide-y divide-leads-line bg-[#141416] md:mx-3 md:rounded-2xl md:ring-1 md:ring-leads-line">
                  {sub.items.map((lead) => (
                    <LeadRow
                      ...
                      showFullDate={group.showFullDate}
                      onOpen={() => onOpen(lead)}
                      onOpenCar={onOpenCar}
                    />
                  ))}
