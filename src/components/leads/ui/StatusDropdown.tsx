  ...
  return (
    <div className={cn("relative w-full", className)} ref={dropdownRef}>
      <button
        ...
        className="flex items-center justify-between w-full h-11 md:h-10 px-3 py-2 bg-transparent border border-border/60 rounded-[10px] md:rounded-[8px] shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-ring/20 transition-all duration-300 ease-out text-[15px] md:text-[14px] font-medium text-zinc-900 hover:border-border"
      >
        ...
        <div className="absolute z-50 w-full mt-1.5 bg-white/95 backdrop-blur-xl border border-border/40 rounded-[12px] shadow-lg max-h-60 overflow-y-auto animate-in fade-in zoom-in-[0.98] duration-300 ease-out py-1.5 custom-scrollbar">
          {LEAD_STATUSES.map((status) => (
            ...
            >
              ...
              {value === status && <Check className="w-4 h-4 text-zinc-900 stroke-[1.5] animate-in zoom-in-50 duration-300 ease-out" />}
            </button>
          ))}
        </div>
      )}
